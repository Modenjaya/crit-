#!/usr/bin/env node
// swap.mjs — SWAP USDC -> GUSD (vault)
// Deps: npm i @mysten/sui dotenv

// [WAJIB HAPUS/KOMENTARI]: Agar tidak menimpa PRIVATE_KEY dari auto.mjs
...
  // [MODIFIKASI]: Pesan error yang lebih jelas dan umum
  if (!raw) throw new Error('PRIVATE_KEY atau SUI_PRIVATE_KEY kosong di environment.'); 
  
...
// [MODIFIKASI]: Hapus deklarasi kp, scheme, sui, dan address di sini. 
// Pindahkan ke dalam run()
...
async function creekInfo(addr) { return creekFetch(`/api/user/info/${addr}`);} // [MODIFIKASI]: Menerima address
...
async function pickUsableUsdc(address, amountRaw) { // [MODIFIKASI]: Menerima address
...
async function execTxWithFallback(tx, kp) { // [MODIFIKASI]: Menerima kp
  // DRY RUN
...
async function run() {
  // [MODIFIKASI]: Membaca key dari ENV dan melakukan parsing di dalam run()
  const RAW_PK = process.env.PRIVATE_KEY || process.env.SUI_PRIVATE_KEY || '';
  if (!RAW_PK) throw new Error('FATAL: PRIVATE_KEY tidak ditemukan di environment.');
  
  const SUI_KEY_SCHEME = String(process.env.SUI_KEY_SCHEME || 'ed25519').toLowerCase();
  const { kp, scheme } = parsePrivKey(RAW_PK, SUI_KEY_SCHEME);
...
  try { const info = await creekInfo(address); creekBefore = parseUserInfo(info); badgesBefore = parseBadges(info); } catch {}

  const coin = await pickUsableUsdc(address, amountRaw); // [MODIFIKASI]: Kirim address
...
      const exec = await execTxWithFallback(tx, kp); // [MODIFIKASI]: Kirim kp
      if (exec?.dryRunOnly) {
...
      // Creek after
      try {
        const after = await creekInfo(address); // [MODIFIKASI]: Kirim address
